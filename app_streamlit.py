...
include_profile = st.sidebar.checkbox("Профилирование данных", value=True)
    ...
    try:
        ...
        for i, chunk in enumerate(chunks[:5]):
            with st.expander(f"Чанк {i+1}: строки {chunk['source_ref']['row_start']}-{chunk['source_ref']['row_end']}"):
                ...
                # Исправлено: headers теперь список строк
        ...
        if include_profile:
            with st.spinner("Профилирование данных..."):
                try:
                    if uploaded_file.name.endswith(".xlsx"):
                        ...
                        try:
                            ...
                            df = pd.read_csv(tmp_path, encoding="windows-1251", on_bad_lines="skip")
                    
                    profiler = TableProfiler()
                    profile = profiler.profile(df)
                except Exception as e:
                    st.error(f"⚠️ Ошибка профилирования: {e}")
                    profile = None
            ...
            if profile:
                ...
                if profile["warnings"]:
                    st.warning("⚠️ Предупреждения качества:")
                    for w in profile["warnings"]:
                        st.write(f"- {w}")
                
                for col_name, col_profile in profile["columns"].items():
                    with st.expander(f"Колонка: `{col_name}`"):
                        st.write(f"**Тип:** {col_profile.get('dtype', 'unknown')} | **Пропуски:** {col_profile['null_percentage']}% | **Уникальных:** {col_profile['unique_count']}")
